...
import net.didion.jwnl.util.UnicodeUtil;
...
public class FileManagerImpl implements FileManager {
    ...
    /**
     * Reads the first word (UTF8) from a file (ie offset, index word)
     *
     * @param file - the file
     * @return - string
     * @throws IOException
     */
    private String readLineUTFWord(RandomAccessDictionaryFile file) throws IOException {
        ByteBuffer charInput = ByteBuffer.allocate(1024);
        int c;
        while (((c = file.read()) != -1) && c != '\n' && c != '\r' && c != ' ') {
    ...
    private String readUTFLine(RandomAccessDictionaryFile file) throws IOException {
    ...
    public long getMatchingLinePointer(POS pos, DictionaryFileType fileType, long offset, String substring)
        ...
        synchronized (file) {
            file.seek(offset);
            do {
                String line = readLineUTFWord(file);
                long nextOffset = file.getFilePointer();
                if (line == null) {
                    return -1;
                }
    ...
    public long getIndexedLinePointer(POS pos, DictionaryFileType fileType, String target) throws IOException {
        RandomAccessDictionaryFile file = (RandomAccessDictionaryFile) getFile(pos, fileType);
        UnicodeUtil aUtil = new UnicodeUtil();
        if (file == null || file.length() == 0) {
            return -1;
        }
        synchronized (file) {
    ...
    public long getFirstLinePointer(POS pos, DictionaryFileType fileType) throws IOException {
        ...
        RandomAccessDictionaryFile file = (RandomAccessDictionaryFile) getFile(pos, fileType);
        String line = null;
        for (line = null; line == null || line.trim().length() == 0; line = readLineUTFWord(file)) {
